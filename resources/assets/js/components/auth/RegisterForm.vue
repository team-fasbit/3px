<template>
 <form method="post" @submit.prevent="submit" @keydown="clearErrors($event.target.name)">
  ...
  <div class="form-group has-feedback " :class="{ 'has-error': form.errors.has('phone') }">
   <input type="text" class="form-control" :placeholder="trans('adminlte_lang_message.phone')" name="phone" value="" v-model="form.phone" autofocus/>
  ...
  methods: {
    submit() {
      this.form
        ...
        .catch(error => {
          console.log(
            this.trans("adminlte_lang_message.registererror") + ":" + error
          );
        });
    },
    clearErrors(name) {
